...
KH1_start <- str_which(KH1_raw, fixed("Sora: I've been having these weird thoughts lately... Like, is any of this"))
...
KH1 <- data_frame(KH1_sub = KH1_sub) %>%
  # Filtering out empty lines
  filter(KH1_sub != "") %>%
  mutate(level = str_detect(KH1_sub, "\\["),
         scene = cumsum(level)) %>%
